...
function renderScoutAgent() {
  ...
  container.innerHTML = `
    ...
    <!-- Active Scouts (top) -->
    <div class="scout-active-section">
      <div class="scout-active-header">
        <div class="scout-active-header-left">
          <h2 class="scout-section-title">Active Scouts</h2>
          <p class="scout-section-subtitle">Your saved searches and match notifications</p>
        </div>
        <button class="btn btn-primary btn-sm scout-new-btn" onclick="toggleNewScoutPanel()" id="newScoutBtn">
          <i data-lucide="plus" width="16" height="16"></i> New Scout
        </button>
      </div>
    ...
    <!-- New Scout Panel (collapsed by default) -->
    <div class="scout-new-panel" id="scoutNewPanel">
      <div class="scout-new-panel-header">
        <h2 class="scout-new-panel-title">Configure New Scout</h2>
        <button class="btn btn-ghost btn-sm" onclick="toggleNewScoutPanel()">
          <i data-lucide="x" width="16" height="16"></i> Close
        </button>
      </div>
...
function generatePdfAnalysis(id, type) {
  // Gather lead data from either source
  let lead;
  if (type === 'api') {
    lead = apiLeads.find(l => l.id === id);
  } else {
  ...
  if (!lead) { showToast('Error', 'Lead not found'); return; }

  // Normalise fields across api leads and PARCELS
  const address   = type === 'api' ? (lead.address + (lead.city ? ', ' + lead.city : '')) : lead.address;
  const county    = lead.county || '';
  const state     = lead.state || 'MI';
  const acreage   = lead.acreage || 0;
  const price     = type === 'api' ? (lead.asking_price || 0) : (lead.askingPrice || 0);
  const zoning    = lead.zoning || 'N/A';
  const rating    = type === 'api' ? (lead.feasibility_rating || 'Unrated') : (lead.feasibilityRating || 'Unrated');
  const lotYield  = lead.lot_yield || lead.lotYield || 0;
  const pricePerAcre = acreage > 0 ? Math.round(price / acreage) : 0;
  const notes     = type === 'api' ? (lead.notes || '') : (lead._notes || '');
  const listingUrl = lead.listing_url || lead.listingUrl || '';
  const stage     = lead.stage || lead.pipeline || 'new_lead';
  const stageLabel = PIPELINE_STAGES.find(s => s.id === stage)?.label || stage;
  const owner     = lead.owner || '';
  const apn       = lead.apn || '';
  const township  = lead.township || '';
  const landValue = lead.landValue || 0;
  const totalValue = lead.totalValue || 0;
  const lastSalePrice = lead.lastSalePrice || 0;
  const lastSaleDate  = lead.lastSaleDate || '';
  const schoolDistrict = lead.schoolDistrict || '';
  const zoningFull = lead.zoningFull || zoning;
  const minLotSize = lead.minLotSize || '';
  const minFrontage = lead.minFrontage || '';
  const frontSetback = lead.frontSetback || '';
  const sideSetback = lead.sideSetback || '';
  const rearSetback = lead.rearSetback || '';
  const maxCoverage = lead.maxCoverage || '';
  const floodZone = lead.floodZone || '';
  const floodRisk = lead.floodRisk || '';
  const wetlands  = lead.wetlands || '';
  const topography = lead.topography || '';
  const soilType  = lead.soilType || '';
  const permittedUses = lead.permittedUses || [];
  const grossProfit = lead.gross_profit_pct || lead.grossProfitPct || 0;
  const maxLandPrice = lead.max_land_price || lead.maxLandPrice || 0;
  ...
  // Colours
  const RED   = [192, 48, 48];
  const GREEN = [43, 94, 58];
  const GOLD  = [212, 168, 67];
  const DARK  = [26, 26, 26];
  const MUTED = [107, 107, 101];
  const BORDER = [224, 224, 218];
  const BG    = [245, 245, 240];

  // ── Embedded logo (SquareBerry logo-sidebar.png) ──
  const LOGO_DATA = 'data:image/png;base64,';

  function setColor(rgb) { doc.setTextColor(rgb[0], rgb[1], rgb[2]); }
  function drawLine(x1, yy, x2) { doc.setDrawColor(BORDER[0], BORDER[1], BORDER[2]); doc.setLineWidth(0.3); doc.line(x1, yy, x2, yy); }
  function addRow(label, value, yy) {
    setColor(MUTED); doc.setFontSize(9); doc.text(label, marginL, yy);
    setColor(DARK); doc.setFont('helvetica', 'bold'); doc.text(value, W - marginR, yy, { align: 'right' }); doc.setFont('helvetica', 'normal');
    drawLine(marginL, yy + 1.5, W - marginR);
    return yy + 7;
  }
  function checkPage(need) { if (y + need > H - 20) { doc.addPage(); y = 18; } }
  function sectionTitle(text) {
    checkPage(14);
    y += 4;
    setColor(DARK); doc.setFont('helvetica', 'bold'); doc.setFontSize(12); doc.text(text, marginL, y);
    doc.setFont('helvetica', 'normal');
    drawLine(marginL, y + 2, W - marginR);
    y += 8;
  }

  // ── Header with logo ──
  try { doc.addImage(LOGO_DATA, 'PNG', marginL, y - 4, 10, 8); } catch(e) { /* ignore */ }
  setColor(RED); doc.setFont('helvetica', 'bold'); doc.setFontSize(16);
  doc.text('SquareBerry', marginL + 13, y + 2);
  setColor(MUTED); doc.setFont('helvetica', 'normal'); doc.setFontSize(8);
  doc.text('Land Intelligence Report', marginL + 13, y + 6);

  // Right side meta
  doc.setFontSize(8); setColor(MUTED);
  doc.text(`Generated: ${dateStr}`, W - marginR, y - 2, { align: 'right' });
  doc.text('Prepared for: Canyon Harris', W - marginR, y + 2, { align: 'right' });
  doc.text('MHBC · Pro Plan', W - marginR, y + 6, { align: 'right' });

  y += 10;
  doc.setDrawColor(RED[0], RED[1], RED[2]); doc.setLineWidth(0.8); doc.line(marginL, y, W - marginR, y);
  y += 10;

  // ── Title ──
  setColor(DARK); doc.setFont('helvetica', 'bold'); doc.setFontSize(18);
  const titleLines = doc.splitTextToSize(address, contentW);
  doc.text(titleLines, marginL, y);
  y += titleLines.length * 7 + 2;

  if (apn || township || county) {
    setColor(MUTED); doc.setFont('helvetica', 'normal'); doc.setFontSize(9);
    const sub = [apn ? `APN: ${apn}` : '', township ? `${township} Twp` : '', county ? `${county} County` : '', state].filter(Boolean).join(' · ');
    doc.text(sub, marginL, y); y += 6;
  }

  // ── Badges ──
  y += 2;
  const badges = [acreage ? `${acreage} acres` : '', zoning, price ? fmt(price) : '', stageLabel].filter(Boolean);
  let bx = marginL;
  doc.setFontSize(8);
  badges.forEach(b => {
    const tw = doc.getTextWidth(b) + 6;
    doc.setFillColor(BG[0], BG[1], BG[2]);
    doc.roundedRect(bx, y - 3, tw, 5.5, 1.5, 1.5, 'F');
    setColor(MUTED); doc.text(b, bx + 3, y + 0.5);
    bx += tw + 3;
  });
  y += 10;
  ...
  const snapItems = [
    { label: 'ASKING PRICE', value: price ? fmt(price) : 'N/A' },
    { label: 'PER ACRE', value: pricePerAcre ? fmt(pricePerAcre) : 'N/A' },
    { label: 'EST. LOT YIELD', value: lotYield ? String(lotYield) : 'N/A' },
    { label: 'RATING', value: rating }
  ];
  const boxW = (contentW - 9) / 4;
  snapItems.forEach((item, i) => {
    const bx2 = marginL + i * (boxW + 3);
    doc.setFillColor(BG[0], BG[1], BG[2]);
    doc.roundedRect(bx2, y, boxW, 16, 2, 2, 'F');
    setColor(DARK); doc.setFont('helvetica', 'bold'); doc.setFontSize(11);
    doc.text(item.value, bx2 + boxW / 2, y + 7, { align: 'center' });
    setColor(MUTED); doc.setFont('helvetica', 'normal'); doc.setFontSize(6);
    doc.text(item.label, bx2 + boxW / 2, y + 12, { align: 'center' });
  });
  y += 22;
  ...
  const detailRows = [
    ...
    ['Acreage', acreage ? `${acreage} acres` : 'N/A'],
    landValue ? ['Land Value', fmt(landValue)] : null,
    totalValue ? ['Total Assessed', fmt(totalValue)] : null,
    lastSalePrice ? ['Last Sale', fmt(lastSalePrice)] : null,
    lastSaleDate ? ['Last Sale Date', lastSaleDate] : null,
    schoolDistrict ? ['School District', schoolDistrict] : null,
    township ? ['Township', township] : null,
    ['County', county || 'N/A'],
    ['State', state]
  ].filter(Boolean);
  detailRows.forEach(([label, value]) => { checkPage(8); y = addRow(label, value, y); });

  // ── Zoning section ──
  if (zoningFull || minLotSize || minFrontage) {
    sectionTitle(`Zoning: ${zoningFull}`);
    const zoningRows = [
      minLotSize ? ['Min Lot Size', minLotSize] : null,
      minFrontage ? ['Min Frontage', minFrontage] : null,
      (frontSetback || sideSetback || rearSetback) ? ['Setbacks', `F: ${frontSetback} · S: ${sideSetback} · R: ${rearSetback}`] : null,
      maxCoverage ? ['Max Coverage', maxCoverage] : null
    ].filter(Boolean);
    zoningRows.forEach(([label, value]) => { checkPage(8); y = addRow(label, value, y); });

    if (permittedUses.length > 0) {
      checkPage(10); y += 3;
      setColor(DARK); doc.setFont('helvetica', 'bold'); doc.setFontSize(9); doc.text('Permitted Uses', marginL, y); y += 5;
      doc.setFont('helvetica', 'normal'); doc.setFontSize(8); setColor(MUTED);
      permittedUses.forEach(u => { checkPage(5); doc.text(`✓  ${u}`, marginL + 2, y); y += 4; });
    }
  }

  // ── Feasibility section ──
  if (grossProfit || maxLandPrice || lotYield) {
    sectionTitle('Subdivision Feasibility');
    const feasRows = [
      grossProfit ? ['Gross Profit %', `${grossProfit}%`] : null,
      maxLandPrice ? ['Max Land Price', fmt(Math.round(maxLandPrice))] : null,
      lotYield ? ['Estimated Lot Yield', String(lotYield)] : null,
      ['Feasibility Rating', rating]
    ].filter(Boolean);
    feasRows.forEach(([label, value]) => { checkPage(8); y = addRow(label, value, y); });

    // Verdict box
    checkPage(18); y += 3;
    const verdictColor = rating === 'Strong' ? GREEN : rating === 'Moderate' ? GOLD : RED;
    doc.setFillColor(verdictColor[0], verdictColor[1], verdictColor[2], 0.07);
    doc.roundedRect(marginL, y, contentW, 14, 3, 3, 'F');
    setColor(verdictColor); doc.setFont('helvetica', 'bold'); doc.setFontSize(13);
    doc.text(rating, W / 2, y + 8, { align: 'center' });
    doc.setFont('helvetica', 'normal');
    y += 20;
  }

  // ── Environmental section ──
  if (floodZone || wetlands || topography || soilType) {
    sectionTitle('Environmental Flags');
    const envRows = [
      floodZone ? ['Flood Zone', `${floodZone} — ${floodRisk}`] : null,
      wetlands ? ['Wetlands', wetlands] : null,
      topography ? ['Topography', topography] : null,
      soilType ? ['Soil', soilType] : null
    ].filter(Boolean);
    envRows.forEach(([label, value]) => { checkPage(8); y = addRow(label, value, y); });
  }
  ...
  if (notes) {
    ...
    noteLines.forEach(line => { checkPage(5); doc.text(line, marginL, y); y += 4.5; });
  }

  // ── Listing URL ──
  if (listingUrl) {
    checkPage(10); y += 4;
    setColor(MUTED); doc.setFontSize(8); doc.text('Listing URL: ', marginL, y);
    setColor(RED); doc.textWithLink(listingUrl, marginL + 18, y, { url: listingUrl });
    y += 6;
  }

  // ── Footer ──
  checkPage(16); y += 6;
  doc.setDrawColor(BORDER[0], BORDER[1], BORDER[2]); doc.setLineWidth(0.5); doc.line(marginL, y, W - marginR, y);
  y += 6;
  setColor(MUTED); doc.setFontSize(7); doc.setFont('helvetica', 'normal');
  doc.text(`Generated by SquareBerry Land Intelligence · ${dateStr} · Confidential`, W / 2, y, { align: 'center' });
  doc.text('This report is for informational purposes only. Verify all data independently before making investment decisions.', W / 2, y + 4, { align: 'center' });

  // ── Save + Download ──
  const filename = `SquareBerry-Analysis-${address.replace(/[^a-zA-Z0-9]/g, '-').substring(0, 60)}.pdf`;
  doc.save(filename);

  // Record in reports API
  try {
    fetch(`${API_BASE}/api/reports`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        title: `${address} — PDF Analysis`,
        address: address,
        acreage: acreage,
        county: county,
        feasibility_rating: rating,
        file_size: `${Math.round(doc.output('arraybuffer').byteLength / 1024)} KB`,
        report_date: new Date().toISOString().slice(0, 10)
      })
    }).then(() => { fetchApiReports(); renderReports(); });
  } catch (e) { console.log('Could not save report to API'); }

  closeLeadDetail();
  showToast('PDF Downloaded', `Analysis report for ${address} saved to downloads`);
}
...
function toggleNewScoutPanel() {
  const panel = document.getElementById('scoutNewPanel');
  const btn = document.getElementById('newScoutBtn');
  if (!panel) return;
...
window.toggleNewScoutPanel = toggleNewScoutPanel;
